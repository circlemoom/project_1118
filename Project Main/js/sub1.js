...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function observerCallback(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 흰색으로 설정할 섹션
                if (["brand_story", "footer_fst", "footer_snd"].includes(entry.target.id)) {
                    backtopIcon.style.filter = "brightness(1)"; // 흰색
                }
                // 검정색으로 설정할 섹션
                else if (["banner", "our_service"].includes(entry.target.id)) {
    ...
    // 각 섹션을 감시
    observer.observe(document.getElementById("brand_story"));
    ...
    observer.observe(document.getElementById("our_service"));
});
